{pkgs, ...}: {
  me.agentForwardingTmux.homeManager = {pkgs, ...}: {
    home.file.sshRc = {
      target = ".ssh/rc";
      text = ''
        #!/bin/bash
        ...
        # rewriting sshrc means that X-forwarding no longer works. The code below
        # is taken from the `sshd` manual.
        # https://unix.stackexchange.com/a/231301
